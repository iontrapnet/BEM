...
const md5_uint8_t MD5::fillbuf[64] = {
...
void MD5::finish()
{
  ...
  *reinterpret_cast<md5_uint32_t*>(&ibuffer[bytes + pad]) = total[0] << 3;
  *reinterpret_cast<md5_uint32_t*>(&ibuffer[bytes + pad + 4]) = (total[1] << 3) | (total[0] >> 29);
 ...
 */
void MD5::update(const md5_uint8_t* buf, unsigned int len)
{
  ...
  if (buflen != 0)
  {
    ...
    buf = static_cast<const md5_uint8_t*>(buf) + add;
    len -= add;
  }
  ...
  if (len >= 64)
  {
    ...
    // don't.
      #if __GNUC__ >= 2
        #define UNALIGNED_P(p) (((md5_uint32_t) p) % __alignof__(md5_uint32_t) != 0)
      #else
        #define UNALIGNED_P(p) (((md5_uint32_t) p) % sizeof(md5_uint32_t) != 0)
      #endif  // __GNUC__ >= 2
    ...
    if (UNALIGNED_P(buf))
      while (len > 64)
      {
        ...
        buf = static_cast<const md5_uint8_t*>(buf) + 64;
        len -= 64;
      }
    else
    #endif // !_STRING_ARCH_unaligned
    {
      ...
      buf = static_cast<const md5_uint8_t*>(buf) + (len & ~63);
      len &= 63;
    }
  }
...
{
  md5_uint32_t correct_words[16];
  const md5_uint32_t* words = static_cast<const md5_uint32_t*>(buf);
  size_t nwords = len / sizeof(md5_uint32_t);
  const md5_uint32_t* endp = words + nwords;
  md5_uint32_t A = A_;
  md5_uint32_t B = B_;
  md5_uint32_t C = C_;
  md5_uint32_t D = D_;
  ...
  while (words < endp)
  {
    md5_uint32_t* cwp = correct_words;
    md5_uint32_t  A_save = A;
    md5_uint32_t  B_save = B;
    md5_uint32_t  C_save = C;
    md5_uint32_t  D_save = D;
...
void* MD5::getValue(void* buffer) const
{
  ...
  (reinterpret_cast<md5_uint32_t*>(buffer))[0] = md5.A_;
  (reinterpret_cast<md5_uint32_t*>(buffer))[1] = md5.B_;
  (reinterpret_cast<md5_uint32_t*>(buffer))[2] = md5.C_;
  (reinterpret_cast<md5_uint32_t*>(buffer))[3] = md5.D_;
