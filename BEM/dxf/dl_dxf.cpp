...
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface,
    ...
    // Indicates start of new entity or var
        ...
        case DL_ENTITY_SEQEND:
            ...
            break;
		case DL_ENTITY_3DFACE://changed by KS
			add3DFace(creationInterface);
			break;
		case DL_BLOCK_RECORD://changed by KS
			addBlockRecord(creationInterface);
			break;
        default:
            break;
        }
        ...
        } else if (!strcmp(groupValue, "3DFACE")) { //Changed by KS
            currentEntity = DL_ENTITY_3DFACE;
        } else if (!strcmp(groupValue, "BLOCK_RECORD")) { //Changed by KS
            currentEntity = DL_BLOCK_RECORD;
        }  else {
            currentEntity = DL_UNKNOWN;
        }
...
//changed by KS
void DL_Dxf::addBlockRecord(DL_CreationInterface* creationInterface) {
    DL_BlockRecordData d(toInt(values[5]),
                  toString(values[2])
				  );

    creationInterface->addBlockRecord(d);
}
...
void DL_Dxf::addPolyline(DL_CreationInterface* creationInterface) {
    DL_PolylineData pd(maxVertices, toInt(values[71], 0), toInt(values[72], 0), toInt(values[70], 0),toInt(values[330],0));
