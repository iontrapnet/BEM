...
struct DL_PolylineData {
     ...
     * Parameters: see member variables.
     */
    DL_PolylineData(int pNumber, int pMVerteces, int pNVerteces, int pFlags,int pOwner) {
        number = pNumber;
        m = pMVerteces;
        n = pNVerteces;
        flags = pFlags;
...
struct DL_3DFaceData {
     ...
     * Parameters: see member variables.
     */
    DL_3DFaceData(double ax,double ay,double az,
					double bx,double by,double bz,
					double cx,double cy,double cz,
					double dx,double dy,double dz,int owner):ax(ax),ay(ay),az(az),
													bx(bx),by(by),bz(bz),
													cx(cx),cy(cy),cz(cz),
													dx(dx),dy(dy),dz(dz),owner(owner){}				
	double ax;
	double ay;
	double az;
    double bx;
 ...
 * @author KS
 */
struct DL_BlockRecordData {
    /**
     * Constructor.
     * Parameters: see member variables.
     */
    DL_BlockRecordData(int handle,string name):handle(handle),name(name){}				
	int handle;
	string name;
